...
if __name__ == "__main__":
    ...
    # # Write CSVs
    # out_root = Path("output_cases_5to7_TW")
    # out_root.mkdir(exist_ok=True)
    # to_teraW(master).to_csv(out_root / "synthetic_teraW_5to7_all.csv", index=False)
    # to_reimink(master).to_csv(out_root / "synthetic_reimink_5to7_all.csv", index=False)
    # master.to_csv(out_root / "synthetic_wetherill_5to7_all.csv", index=False)

    # print(f"✓ wrote {len(master)} rows to {out_root}")

    # # Linearity quick check (optional)
    # print("\nLinearity check:")
    # for df_panel in panels:
    #     lin = linearity_L(df_panel)
    #     c, t = df_panel['Case'].iat[0], df_panel['Tier'].iat[0]
    #     print(f"  Case {c}, Tier {t}: inliers {lin:.2f}")
